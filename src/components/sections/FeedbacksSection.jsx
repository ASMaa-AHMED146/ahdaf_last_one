...
export default function FeedbacksSection() {
    ...
    return (
        <div 
            className={`p-[50px] flex relative h-[500px] md:mt-[50px] mt-[20px] ${isArabic ? 'rtl' : 'ltr'}`}
            style={{
                backgroundImage: `url(${img})`,
                backgroundPosition: 'center',
                backgroundSize: 'cover',
        ...
        >
            ...
            <div className={`flex absolute w-full h-full top-0 ${isArabic ? 'left-0' : 'right-0'} z-30 items-center justify-center`}>
                <button 
                    onClick={() => swiper?.slidePrev()} 
                    className="focus:outline-none md:mx-0 mx-[10px]"
                >
                    {isArabic ? (
                        <SlArrowLeft className='text-[40px] hover:scale-110 transition-transform cursor-pointer' style={{ color: "#FE8400" }} />
                    ) : (
