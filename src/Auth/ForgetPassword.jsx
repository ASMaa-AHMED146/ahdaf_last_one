...
export default function Forgetpassword() {
  ...
  const {
    ...
    trigger,
    formState: { errors },
    getValues // إضافة getValues للحصول على قيم الفورم
  } = useForm({
    mode: "onBlur",
    resolver: zodResolver(forgetPasswordSchema),
  });

  // نقل useEffect خارج submitForm
  useEffect(() => {
    dispatch(resetUI());
    return () => {
      dispatch(resetUI());
  ...
  const submitForm = async (data) => {
    ...
    try {
      const result = await dispatch(actEmailVerfy(data)).unwrap();
      
      // في حالة النجاح، انتقل لصفحة set password مع تمرير الإيميل
      navigate('/auth/setpassword', { 
        state: { 
          email: data.email // تمرير الإيميل للصفحة التالية
        } 
      });
    } catch (error) {
