...
export default function Aboutus() {
  ...
  return (
    <div>
      <Heading service={t("aboutus.pageTitle")}/>
      <div className='lg:px-[70px] lg:pt-[70px] px-[30px] pt-[20px] '>
        <div className='rounded-[10px] overflow-hidden lg:h-[550px] h-[220px]  lg:py-[50px] lg:px-[100px] relative lg:mb-[50px]'><img className=' w-full absolute top-[-110px] right-[0px]' src={firstitem.image || main} alt='about img'/></div>
        <div className=''>
          <p className='border-b-[3px] border-b-[#F36C35] font-[400] text-[32px] w-fit mb-[15px]'>{t("aboutus.whoWeAre")}</p>
          <p className='font-[300] lg:text-[29px] text-[25px] '>{isArabic ? firstitem.title_ar : firstitem.title_en}</p>
        </div>
        <div className='mt-[15px]'>
          {
            combinedData.map((el,index)=>(
        ...
        <div className='py-[30px]'>
          <p className='flex items-center gap-[5px] text-[22px]'>{t("aboutus.contactUs")} :<FaPhoneAlt className='text-[#707070]' style={{color:'#F36C35'}}/> <span className='text-[#707070]'>01090009330</span></p>
          <button className='border-[1px] rounded-[8px] px-[15px] py-[10px] text-[#707070] mt-[15px] transform transition-all duration-300 ease-in-out hover:scale-110 active:scale-95 cursor-pointer' onClick={()=>nav('/booking')}>{t("aboutus.bookService")}</button>
        </div>
        <Company sectitem={sectitem}/>
      </div>
    </div>
  ...
  return(
    <>
      <div className='flex gap-[20px] lg:flex-row flex-col'>
        ...
        <div className='lg:max-w-[50%]'>
          <h1 className='border-b-[3px] border-b-[#1A7474] font-[450] text-[32px] w-fit '>{t("aboutus.aboutCompany")}</h1>
          <p className='my-[15px] text-[24px] font-[400]'>{isArabic ? sectitem.title_ar : sectitem.title_en}</p>
          <p className='flex items-center gap-[5px] text-[#F36C35] text-[25px]'><FaRegCircleCheck className='text-[20px]' style={{color:'#F36C35'}}/>{isArabic ? sectitem.subtitles_ar : sectitem.subtitles_en}
              </p>
                      <p className='text-[#707070] lg:text-[23px] leading-[40px]'>{isArabic ? sectitem.descriptions_ar : sectitem.descriptions_en}</p>
        </div>
